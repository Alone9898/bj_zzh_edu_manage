<template>
  <div class="incomeTable">
    ...
    <div class="headerBox">
      ...
      <div>
        <div>税前金额总计</div>
        <div>{{ tableData.tax_total_money }}</div>
      </div>
      <div>
        <div>扣缴税额总计</div>
        <div>{{ tableData.after_tax_count }}</div>
      </div>
      <div>
        <div>税后金额总计</div>
...
export default {
  ...
  methods: {
    ...
    init(){
      ...
      this.axios
          .post("platform/Doctorstatistics/getDoctorRevenueList", data)
          .then((res) => {
            ...
            this.tableData.list.forEach(item => {
              ...
              if (item.sa_time) {
                item.sa_time = this.dayjs(item.sa_time * 1000).format('YYYY/MM/DD HH:mm')
              }
              if(item.type){
                switch (item.type){
                  case 1:
